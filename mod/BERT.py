...
class embeds(nn.Module):
    ...
    def E(self, text, level=-1, clip_at=500):
        ...
        outputs = self.mod(torch.LongTensor(ids[steps[0][0]:steps[0][1]]).unsqueeze(0))[2][level].squeeze(0)

        if len(steps) > 1:
            for step in steps[1:5]:
                outputs = torch.cat([outputs, self.mod(torch.LongTensor(ids[step[0]:step[1]]).unsqueeze(0))[2][level].squeeze(0)], dim=0)
